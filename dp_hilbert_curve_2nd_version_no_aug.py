...
def process_1d_tsc_data(x_train_norm, x_test_norm, y_train, y_test, output_dir):
    ...
    X_train_padded_2d = x_train_standardized[:, np.newaxis, :, :]
    X_test_padded_2d = x_test_standardized[:, np.newaxis, :, :]
    ...
    # np.save(os.path.join(output_dir, 'X_train_1d.npy'), X_train_1d_np)
    # np.save(os.path.join(output_dir, 'X_test_1d.npy'), X_test_1d_np)
...
def process_dataset():
    ...
    # x_train_aug = data_aug(x_train[..., np.newaxis])
    ...
    process_1d_tsc_data(x_train_norm, x_test_norm, y_train, y_test, output_dir)
